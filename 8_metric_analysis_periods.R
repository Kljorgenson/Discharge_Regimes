...
inds.20$ecoregion1 <- PCA_dat.20$ecoregion1

# View mean PCA indices for periods by groups
inds2 <- left_join(inds.20,meta_full)
inds2 %>% group_by(period) %>% summarise(pc1 = mean(Dim.1), pc2 = mean(Dim.2), pc3 = mean(Dim.3))
a <- inds2 %>% group_by(ecoregion1, period) %>% summarise(pc1 = mean(Dim.1), pc2 = mean(Dim.2), pc3 = mean(Dim.3))
b <- inds2 %>% group_by(permafrost_class, period) %>% summarise(pc1 = mean(Dim.1), pc2 = mean(Dim.2), pc3 = mean(Dim.3))
c <- inds2 %>% group_by(glacial, period) %>% summarise(pc1 = mean(Dim.1), pc2 = mean(Dim.2), pc3 = mean(Dim.3))
e <- inds2 %>% group_by(glacial) %>% summarise(pc1 = mean(Dim.1), pc2 = mean(Dim.2), pc3 = mean(Dim.3))
d <- rbind(a,b,c)
write.csv(d, 'Output_data/PCA_20_60_inds.csv')
...
p1 <- metric.dat.20 %>% filter(name %in% old$name | name %in% c("COPPERMINE RIVER AT OUTLET OF POINT LAKE", "KUPARUK")) %>% mutate(shape = paste(glacial,period,sep="")) %>%
  ...
  geom_text(aes( x=0.3, y=-2, label="SNR"), col = 1, size=4) +
  geom_text(aes( x=3.1, y=-0.4, label = paste("A[RMS]")), parse = T, col = 1, size=4) +
  geom_text(aes( x=2, y=-0.8, label="HSAM timing"), col = 1, size=4) +
...
p2 <- metric.dat.20 %>% filter(name %in% old$name | name %in% c("COPPERMINE RIVER AT OUTLET OF POINT LAKE", "KUPARUK")) %>% mutate(shape = paste(glacial,period,sep="")) %>%
  ...
  geom_segment(data = coords.20,
               ...
               color = 1, arrow = arrow(angle = 25, length = unit(4, "mm"))) +
  geom_text(aes( x=0.2, y=-2, label="SNR"), col = 1, size=4) +
  geom_text(aes( x=1, y=-0.3, label = paste("A[RMS]")), parse = T, col = 1, size=4) +
  ...
  geom_text(aes( x=0.6, y=1, label=paste('sigma[hf]')), parse = T, col = 1, size=4) +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        axis.title = element_text(size = 14),
        axis.text = element_text(size = 12))
p2
...
p1.1 <- p1 + theme(legend.position = 'bottom') + guides(shape = 'none')

# Extract the shape and color legends individually
legend_shape <- get_legend(p2.2+ theme(legend.text=element_text(size=11.5, margin = margin(l = 0, r =-3, t = 1, unit = "pt")), legend.key.size = unit(0.8,'cm'), legend.margin = margin(t = -.7, unit="cm")))
legend_color <- get_legend(p1.1+ theme(legend.text=element_text(size=11.5, margin = margin(l = 2, r = 3, unit = "pt")), legend.key.size = unit(0.4,'cm'), legend.margin = margin(b = -.3, t = -0.5, unit="cm")))
...
plot_grid(
  ...
  combined_legends,
  ncol = 1,
  rel_heights = c(1, 0.2)) + theme(plot.background = element_rect(fill="white", color = NA))
...
dat6 <- metric.dat.20 %>% dplyr::select(ecoregion1, glacial, permafrost_class, period, name, Dim.1, Dim.2, Dim.3) %>% na.omit() %>% filter(ecoregion1 == "Marine West Coast Forest")
ad6 <- adonis2(dat6[,6:8]~dat6$period, method = "euc", strata = dat6$name)
...
p.values <- c(ad4$`Pr(>F)`[1],ad5$`Pr(>F)`[1],ad6$`Pr(>F)`[1],ad7$`Pr(>F)`[1])
p.e <- data.frame(p = p.values, group = c('Northern Forests', 'Northwestern Forested Mountains', 'Marine West Coast Forest', 'Taiga'))
...
dat9 <- metric.dat.20 %>% dplyr::select(ecoregion1, glacial, permafrost_class, period, name, Dim.1, Dim.2, Dim.3) %>% na.omit() %>% filter(permafrost_class %in% c("D"))
...
p.values <- c(ad8$`Pr(>F)`[1],ad9$`Pr(>F)`[1],ad10$`Pr(>F)`[1],ad11$`Pr(>F)`[1])
p.p <- data.frame(p = p.values, group = c('C', 'D', 'S', 'Unfrozen'))
...
dat %>% group_by(ecoregion, variable) %>% identify_outliers(value)

### Repeated measured ANOVA by ecoregion
## Rain
d1 <- dat %>% filter(variable == 'Rain', ecoregion1 != 'Tundra') %>% arrange(period)
...
rain.p <- emmeans(lme_rain, ~ period | ecoregion1)
rain.pairs <- summary(pairs(rain.p))


## Snowfall
d2 <- dat %>% filter(variable == 'Snowfall', ecoregion1 != 'Tundra') %>% arrange(period)
lme_snowfall <- lme(value ~ ecoregion1 * period,
                random = ~1 | name,
                data = d2)

# Comparison between periods within ecoregions
snowfall.p <- emmeans(lme_snowfall, ~ period | ecoregion1)
snowfall.pairs <- summary(pairs(snowfall.p))


## Temperature
d3 <- dat %>% filter(variable == 'Temperature', ecoregion1 != 'Tundra') %>% arrange(period)
lme_temp <- lme(value ~ ecoregion1 * period,
                    random = ~1 | name,
                    data = d3)

# Comparison between periods within ecoregions
temp.p <- emmeans(lme_temp, ~ period | ecoregion1)
temp.pairs <- summary(pairs(temp.p))

## Snowmelt timing
d4 <- dat %>% filter(variable == 'Snowmelt timing', ecoregion1 != 'Tundra') %>% arrange(period)
lme_snow_t <- lme(value ~ ecoregion1 * period,
                random = ~1 | name,
                data = d4)

# Comparison between periods within ecoregions
snow_t.p <- emmeans(lme_snow_t, ~ period | ecoregion1)
snow_t.pairs <- summary(pairs(snow_t.p))

## Rainfall seasonality index
d5 <- dat %>% filter(variable == 'Rainfall Seasonality Index', ecoregion1 != 'Tundra') %>% arrange(period)
lme_SI <- lme(value ~ ecoregion1 * period,
                  random = ~1 | name,
                  data = d5)

# Comparison between periods within ecoregions
SI.p <- emmeans(lme_SI, ~ period | ecoregion1)
SI.pairs <- summary(pairs(SI.p))


### Stat output dataframe
stats <- full_join(rain.pairs,snowfall.pairs) %>% full_join(temp.pairs) %>% full_join(snow_t.pairs) %>% full_join(SI.pairs) %>% as.data.frame() %>%
  mutate(variable = c(rep('Rain', 4),rep('Snowfall', 4),rep('Temperature', 4),rep('Snowfall timing', 4),rep('Rainfall Seasonality Index', 4)))
...
### Plot of difference in climate between time periods
# Reformat data
dat2 <- dat %>% 
...
a <- dat3 %>% filter(!variable %in% c('Temperature', 'Precipitation', 'Snowmelt timing')) %>% 
  ...
  geom_point(aes(x = 0.85,y=40), shape = 8, col = "#D95F02") + # Add asterisk for ecoregions with significant change between periods
...
# Absolute difference for temperature
b <- dat3 %>% filter(variable == 'Temperature') %>% ggplot(aes(variable, diff, col = ecoregion1))+
  ...
  geom_point(aes(x = 0.85,y=0.2), shape = 8, col = "#D95F02") + # Add asterisk for ecoregions with significant change between periods
...
c <- dat3 %>% filter(variable == 'Snowmelt timing') %>% ggplot(aes(str_wrap(variable, width = 10), diff, col = ecoregion1))+
  ...
  geom_point(size = 0.7, position = position_jitterdodge(jitter.width = 0.1), shape = 1) + ylim(-0.3,0.3) + scale_y_continuous(position = "left") + 
  theme(legend.key.size = unit(0.7, 'cm'))  +
  geom_point(aes(x = 1,y=-15), shape = 8, col = "#66A61E") # Add asterisk for ecoregions with significant change between periods
c
