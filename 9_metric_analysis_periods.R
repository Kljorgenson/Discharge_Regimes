...
rain.pairs <- summary(pairs(rain.p), infer = c(TRUE, TRUE))


## Snowfall
d2 <- dat %>% filter(variable == 'Snowfall') %>% arrange(period)
...
snowfall.pairs <- summary(pairs(snowfall.p), infer = c(TRUE, TRUE))


## Temperature
d3 <- dat %>% filter(variable == 'Temperature') %>% arrange(period)
...
temp.pairs <- summary(pairs(temp.p), infer = c(TRUE, TRUE))

## Snowmelt timing
d4 <- dat %>% filter(variable == 'Snowmelt timing') %>% arrange(period)
...
snow_t.pairs <- summary(pairs(snow_t.p), infer = c(TRUE, TRUE))

## Rainfall seasonality index
d5 <- dat %>% filter(variable == 'Rainfall Seasonality Index') %>% arrange(period)
